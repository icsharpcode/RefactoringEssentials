...
namespace RefactoringEssentials
{
    ...
    class SpeculationAnalyzer
    {
        ...
        public static bool SymbolInfosAreCompatible(SymbolInfo originalSymbolInfo, SymbolInfo newSymbolInfo, bool performEquivalenceCheck, bool requireNonNullSymbols = false)
        {
            try
            {
                return (bool)RoslynReflection.AbstractSpeculationAnalyzer_8.SymbolInfosAreCompatibleMethod.Invoke(null, new object[] { originalSymbolInfo, newSymbolInfo, performEquivalenceCheck, requireNonNullSymbols });
            }
        ...
        public bool SymbolsForOriginalAndReplacedNodesAreCompatible()
        {
            try
            {
                return (bool)RoslynReflection.AbstractSpeculationAnalyzer_8.SymbolsForOriginalAndReplacedNodesAreCompatibleMethod.Invoke(instance, new object[0]);
            }
            catch (TargetInvocationException ex)
            {
                ExceptionDispatchInfo.Capture(ex.InnerException).Throw();
        ...
        public bool ReplacementChangesSemantics()
        {
            try
            {
                return (bool)RoslynReflection.AbstractSpeculationAnalyzer_8.ReplacementChangesSemanticsMethod.Invoke(instance, new object[0]);
            }
            catch (TargetInvocationException ex)
            {
                ExceptionDispatchInfo.Capture(ex.InnerException).Throw();
